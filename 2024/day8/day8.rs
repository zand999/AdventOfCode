    ...
    for i in 0..towers.len(){
        ...
        while j < towers.len() {
            if towers[j].t_type != towers[i].t_type{
                ...
                let dead_zone_1_x:i64 = towers[i].x - x_diff;
                let dead_zone_1_y:i64 = towers[i].y - y_diff;
                let dead_zone_2_x:i64 = towers[j].x + x_diff;
                let dead_zone_2_y:i64 = towers[j].y + y_diff;

                if inBounds((tower_grid[0].len()) as i64, (tower_grid.len()) as i64,dead_zone_1_x,dead_zone_1_y){
                    
                    antinode_grid[dead_zone_1_y as usize][dead_zone_1_x as usize] = '#';
                    //tower_grid[dead_zone_1_y as usize][dead_zone_1_x as usize] = '#';
                    
                }
                if inBounds((tower_grid[0].len()) as i64,(tower_grid.len()) as i64,dead_zone_2_x,dead_zone_2_y){
                    //total += 1;
